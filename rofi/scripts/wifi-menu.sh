...
CHENTRY=$(echo -e "$TOGGLE\nCancel\n$LIST" | uniq -u | rofi -dmenu -selected-row 2 -config "./theme.rasi")

# check exit code - rofi returns 1 when clicking outside or pressing escape
if [ $? -ne 0 ]; then
    exit
fi
...
if [ "$CHENTRY" = "" ] || [ "$CHENTRY" = "Cancel" ]; then
    exit
	...
	if [[ $(echo "$KNOWNCON" | grep "$CHSSID") = "$CHSSID" ]]; then
		...
		if [[ "$CHENTRY" =~ "" ]]; then
			...
			# check if password prompt was cancelled
			if [ $? -ne 0 ]; then
				exit
			fi
		fi
